...
int madvise(uint64 base, uint64 len, int advice) {
  ...
  if (advice == MADV_NORMAL) {
    ...
    for (uint64 va = begin; va <= last; va += PGSIZE) {
      ...
      if (pte != 0 && (*pte & PTE_S)) {
        // if this PTE is swapped, swap in and assign PA to it
...
void vmprintR(pagetable_t pagetable, int level, char *prefix0, char *prefix1, uint64 lastva) {
  if (level == 0)
    printf("page table %p\n", pagetable);

  // find last PTE index
  int last;
  for (last = 511; last >= 0; last--) {
    pte_t *pte = &pagetable[last];
    if (*pte & PTE_V) {
      break;
    }
  }
  ...
  for (int i = 0; i < last+1; i++) {
    ...
    if ((*pte & PTE_V) || (*pte & PTE_S)) {
      if (level >= 1) printf("%s", prefix0);
      if (level >= 2) printf("%s", prefix1);
      ...
      if ((*pte & PTE_D) && (*pte & PTE_V)) printf(" D");
      if ((*pte & PTE_P) && (*pte & PTE_V)) printf(" P");
      ...
      if ((*pte & (PTE_W|PTE_R|PTE_X)) == 0) {
        if (level == 0) {
          vmprintR((pagetable_t)PTE2PA(*pte), level+1, (i == last) ? "    ":"|   ", "", va);
        }
        else if (level == 1) {
          vmprintR((pagetable_t)PTE2PA(*pte), level+1, prefix0, (i == last) ? "    ":"|   ", va);
        }
      }
    }
  }
