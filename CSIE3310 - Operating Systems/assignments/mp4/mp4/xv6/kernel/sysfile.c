...
sys_open(void)
{
  ...
  if ((ip->type == T_SYMLINK) && !(omode & O_NOFOLLOW)){
    // find the target path recuresively
    int count = 0;
...
void traverse_dir(char *cwd, struct inode *root, char *target, char *buf) {
  struct dirent de;

  // traverse all dirents
  ilock(root);
  for (int off = 0; off < root->size; off += sizeof(de)) {
    if (readi(root, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
      panic("sys_revreadlink: readi");
    ...
    // get inode from inum
    struct inode *ip = iget(root->dev, de.inum);
    ilock(ip);

    if (ip->type == T_SYMLINK) {
      // find target of this symlink
      ...
      // check if target is the same as the input target

      if (namecmp(path, target) == 0) {
        // append path to buf
        char pathname[MAXPATH];
        memset(pathname, 0, sizeof(pathname));
        strcat(pathname, cwd);
        strcat(pathname, de.name);
      ...
      strcat(dirname, cwd);
      strcat(dirname, de.name);
      strcat(dirname, "/");

      iunlock(ip);
      traverse_dir(dirname, ip, target, buf);
    }
    else {
      iunlock(ip);
    }
...
sys_revreadlink(void)
{
  ...
  // traverse all inodes from root
  struct inode *root = namei("/");

  traverse_dir("/", root, target, userbuf);
  ...
  // remove tailed space
  int sizeused = strlen(userbuf);
  userbuf[sizeused] = '\0';
  sizeused--;

  // printf("userbuf:%s, sizeused:%d\n", userbuf, sizeused);
