...
// are listed in ip->addrs[].  The next INDIRECT_SIZE blocks are
...
bmap(struct inode *ip, uint bn)
{
  ...
  if (bn < INDIRECT2_SIZE) {
    ...
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;
    if ((addr = a[bn / INDIRECT_SIZE]) == 0) {
      a[bn / INDIRECT_SIZE] = addr = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);

    // Load block
    bn -= (bn / INDIRECT_SIZE) * INDIRECT_SIZE;
